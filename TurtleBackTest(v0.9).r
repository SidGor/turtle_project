...
#"cu" "al" "zn" "au" "rb" "ru" "c"  "m"  "a"  "y"  "p"  "l"  "SR" "CF"
#从10年1月1日到16年12月30日有完整数据的
...
#出于sys2判定55日通道突破的需要，这里要求有效数据至少要有56行否则会报错。
if (nrow(data[[i]]) < 56)  {
...
for (i in 1:length(product_ids)) {
  ...
  close_list[[i]][,DailyReturn := c(NA,diff(data[[i]][,close]))
                  /data[[i]][,shift(close,1,type = "lag")]]
}
...
return_table <- return_table[2:.N,] #去NA
...
###############################################################################  
  ...
  test_1 = abs(position) - threshold_1             #大于0则风险溢出
  
  while(max(na.exclude(test_1)) > 0){
    
    a_items_1 <- product_ids[(test_1 >0)]          #找出溢出产品
    ...
    pick_1  =  match(a_close_1$product_name,product_ids) 
                                                #找出到底哪个产品需要反向平仓
    holding[pick_1] = holding[pick_1] + 
                      judgement[pick_1] * a_close_1$no_contract 
                                                         #改合约数
    
    
    #记账-更新trades里面的信息
    trade_id_t1    = a_close_1[, trade_id]
    item_t1        = product_ids[pick_1]
    enter_date_t1  = a_close_1[, enter_date]
    enter_price_t1 = a_close_1[, enter_price]
    leave_date_t1  = cdt[ptr, date]
    leave_price_t1 = cdt[[3 + (pick_1 - 1) * 15]][ptr] - slippage[pick_1]#开仓价平仓 
    ori_direction_t1 = "test1"
    commision_t1   = leave_price_t1 * vm[pick_1] + enter_price_t1 * vm[pick_1]
    profit_t1      = -judgement[pick_1] * (leave_price_t1 - enter_price_t1) * 
                  vm[pick_1] * a_close_1$no_contract 
    contracts_t1   = a_close_1[,no_contract]
    
    closed_profit = closed_profit + profit_t1
    
    fee         = fee + leave_price_t1 * vm[pick_1] * a_close_1$no_contract * 
                  fee.rate[pick_1] + fee_rate_fix[pick_1]
    
    cash        = cash#暂不更新

    WriteTrades(trade_id_t1, item_t1, enter_date_t1, enter_price_t1, leave_date_t1,
                leave_price_t1, ori_direction_t1, commision_t1, profit_t1,contracts_t1) 
  ...
  while(na.exclude(max(test_2)) > 0){
    ...
    #记账-与test1基本相同
    trade_id_t2    = a_close_2[, trade_id]
    item_t2        = product_ids[pick_2]
    enter_date_t2  = a_close_2[, enter_date]
    enter_price_t2 = a_close_2[, enter_price]
    leave_date_t2  = cdt[ptr, date]
    leave_price_t2 = cdt[[3 + (pick_2 - 1) * 15]][ptr] - slippage[pick_2]    
    ori_direction_t2 = "test2"
    commision_t2   = leave_price_t2 * vm[pick_2] + enter_price_t2 * vm[pick_2]
    profit_t2      = -judgement[pick_2] * (leave_price_t2 - enter_price_t2) * 
                      vm[pick_2] * a_close_2$no_contract 
    contracts_t2   = a_close_2[,no_contract]
    closed_profit = closed_profit + profit_t2
    
    fee         = fee + leave_price_t2 * vm[pick_2] * a_close_2$no_contract * 
                  fee.rate[pick_2] + fee_rate_fix[pick_2]
    
    cash        = cash
    
    WriteTrades(trade_id_t2, item_t2, enter_date_t2, enter_price_t2, leave_date_t2,
                leave_price_t2, ori_direction_t2, commision_t2, profit_t2,contracts_t2) 
  ...
  while(max(test_3) > 0){
    ...
    a_items_3 <- product_ids[as.logical(corr_mat[[2]][,test_3>0])]  
    
    a_close_3 <- sta_contract_dt[product_name %in% a_items_3,][.N,]
    
    pick_3  =  match(a_close_3$product_name,product_ids)
    
    holding[pick_3] = holding[pick_3] + 
                      judgement[pick_3] * a_close_3$no_contract #改合约数
    
    #记账
    trade_id_t3    = a_close_3[, trade_id]
    item_t3        = product_ids[pick_3]
    enter_date_t3  = a_close_3[, enter_date]
    enter_price_t3 = a_close_3[, enter_price]
    leave_date_t3  = cdt[ptr, date]
    leave_price_t3 = cdt[[3 + (pick_3 - 1) * 15]][ptr] - slippage[pick_3] 
    ori_direction_t3 = "test3"
    commision_t3   = leave_price_t3 * vm[pick_3] + enter_price_t3 * vm[pick_3]
    
    profit_t3      = -judgement[pick_3] * (leave_price_t3 - enter_price_t3) * 
                  vm[pick_3] * a_close_3$no_contract 
    contracts_t3 = a_close_3[,no_contract]
    
    closed_profit = closed_profit + profit_t3
    fee         = fee + leave_price_t3 * vm[pick_3] * a_close_3$no_contract * 
                  fee.rate[pick_3] + fee_rate_fix[pick_3]
    
    cash        = cash
    
    WriteTrades(trade_id_t3, item_t3, enter_date_t3, enter_price_t3, leave_date_t3,
                leave_price_t3, ori_direction_t3, commision_t3, profit_t3,contracts_t3)
    
    standing_contract[[sta_contract_dt[product_name == 
                      product_ids[pick_3]][.N]$trade_id]] = NULL
  ...
  while(max(test_4) > 0){
    ...
    a_items_4 <- product_ids[(na.fill(position,0) * 
                                sum(na.fill(position,0))) >0] 
   
    a_close_4 <- sta_contract_dt[product_name %in% a_items_4,][.N,]
    
    pick_4  =  match(a_close_4$product_name,product_ids)
    
    holding[pick_4] = holding[pick_4] + judgement[pick_4] * 
                      a_close_4$no_contract #改合约数
    
    #记账
    trade_id_t4    = a_close_4[, trade_id]
    item_t4     = product_ids[pick_4]
    enter_date_t4  = a_close_4[, enter_date]
    enter_price_t4 = a_close_4[, enter_price]
    leave_date_t4  = cdt[ptr, date]
    leave_price_t4 = cdt[[3 + (pick_4 - 1) * 15]][ptr] - slippage[pick_4]  
    ori_direction_t4 = "test4"
    commision_t4   = leave_price_t4 * vm[pick_4] + enter_price_t4 * vm[pick_4]
    profit_t4      = -judgement[pick_4] * (leave_price_t4 - enter_price_t4) * 
                  vm[pick_4] * a_close_4$no_contract
    contracts_t4 = a_close_4[,no_contract]
    
    closed_profit = closed_profit + profit
    
    fee         = fee + leave_price_t4 * vm[pick_4] * a_close_4$no_contract * 
                  fee.rate[pick_4] + fee_rate_fix[pick_4]
    
    cash        = cash
    
    WriteTrades(trade_id_t4, item_t4, enter_date_t4, enter_price_t4, leave_date_t4,
                leave_price_t4, ori_direction_t4, commision_t4, profit_t4,contracts_t4)
    
    standing_contract[[sta_contract_dt[product_name == 
                       product_ids[pick_4]][.N]$trade_id]] = NULL
...
####################Open Position##############################################
  ...
  for (j in 1:length(product_ids)){     #提取对比数据
    
    highs  <- append(highs,cdt[[4+(j-1)*15]][ptr]) 
    highs_55 <- append(highs_55,cdt[[15+(j-1)*15]][ptr]) 
    lows <- append(lows,cdt[[5+(j-1)*15]][ptr])
    lows_55 <- append(lows_55,cdt[[16+(j-1)*15]][ptr])
    ATRs  <- append(ATRs,cdt[[9+(j-1)*15]][ptr]) #需要据此得出开仓倍数
  }  
  ...
  unit_long <- floor((sig_long * highs - sig_long * highs_55)/(0.5*ATRs))  
  #超了几个单位的ATR
  
  unit_long <- unit_long  + sig_long
  #0.5N的情况下应该一共进2个unit，所以整体要根据sig加1或者加0
  ...
  for (j in 1:length(product_ids)){
    ...
    if (is.na(unit_long[j])){ next
      ...
      t_position = copy(position) #不能先改position，建立“虚拟持仓”
      ...
      for(k in 1:unit_long[j]) {
        ...
        enter_price <- max(cdt[[ptr,3 + (j - 1) * 15]], 
                           cdt[[ptr,15 + (j - 1) * 15]]) + slippage[j] 
        fee <- fee + enter_price * units[j] * vm[j] * fee.rate[j] + 
               fee_rate_fix[j]          #update total fee
        cut <- enter_price - 2 * cdt[[9+(j-1)*15]][ptr] #止损点
        trade_id <- paste("|",direction,"|",enter_date,cdt[[2 + 
                          (j-1) * 15]][ptr],"00",k,sep = "")
        ...
        #adding contract to current holding
        
        cash <- cash - enter_price * units[j] * vm[j] - 
          enter_price * units[j] * vm[j] * fee.rate[j] - fee_rate_fix[j]
  ...
  for (j in 1:length(product_ids)){
    ...
    else if (unit_short[j] == 0) next  #节省运算时间,跳过没有开仓计划的产品
    ...
    for(k in 1:unit_short[j]) {
      ...
      #test 2: risk control in buying (matrix 2)
      }else if (any(abs(t_position %*% corr_mat$clscorr) > 6)){
      #test 3: risk control in buying (matrix 3)  
      ...
      #test 4: any direction, total holding should be less than 12  
        ...
        enter_date <- cdt[[1]][ptr]       
        direction <- -1L                 # 1L long, -1L short
        enter_price <- min(cdt[[ptr , 3 + (j-1) * 15]], 
                           cdt[[ptr, 16 + (j-1) *15]]) - slippage[j] 
        ...
        trade_id <- paste("|",direction,"|",enter_date,cdt[[2 + (j-1) * 15]]
                          [ptr],"00",k,sep = "")
        ...
        cash <- cash + enter_price * units[j] * vm[j] - 
                enter_price * units[j] * vm[j] * fee.rate[j] - 
                fee_rate_fix[j]   #update cash
...
####################################数据处理##################################  
names(standing_contract) = list.map(standing_contract, trade_id)
#给每个合约加上合同号索引
  
sta_contract_dt <-  list.stack(standing_contract, data.table = TRUE)   
#use data.frame for easy tracking
...
#集成表格以便判断是否止损。
if(nrow(l_contracts) != 0) {#有记录才进行操作
  
  l_contracts <- as.data.table(
                  left_join(l_contracts, information, by = "product_name"))
...
if(nrow(s_contracts) != 0) {
  s_contracts <- as.data.table(
                  left_join(s_contracts, information, by = "product_name"))
...
if(nrow(s_close) == 0){
  ...
  for (j in 1:nrow(s_close)){
    ...
    leave_price = max(cdt[[ptr, 3 + (product_match - 1) * 15]],
                      s_close[j,cut_point]) + slippage[product_match] 
    ori_direction = s_close[j,direction]
    commision   = leave_price * vm[product_match] + 
                  enter_price * vm[product_match]
    profit      = (enter_price - leave_price) * vm[product_match] * 
                  s_close[j,no_contract] 
    closed_profit = closed_profit + profit
    fee         = fee + leave_price * vm[product_match] * 
                  s_close[j,no_contract] * fee.rate[product_match] + 
                  fee_rate_fix[product_match]
    
    cash        = cash - leave_price * vm[product_match] * s_close[j,no_contract] - 
                  leave_price * vm[product_match] * s_close[j,no_contract] * 
                  fee.rate[product_match] - fee_rate_fix[product_match]
    
    position[product_match] = position[product_match] +1 #adjust position
    holding[product_match] = holding[product_match] + s_close[j,no_contract] 
    #空仓应该反加回去
...
if(nrow(l_close) == 0){
  ...
  for (j in 1:nrow(l_close)){
    ...
    leave_date  = cdt[ptr, date]
    leave_price = min(cdt[[ptr, 3 + (product_match - 1) * 15]], 
                      l_close[j, cut_point]) - slippage[product_match]
    ori_direction = l_close[j, direction]
    commision   = leave_price * vm[product_match] + 
                  enter_price * vm[product_match]
    profit      = (leave_price - enter_price) * vm[product_match] * 
                  l_close[j,no_contract] 
    closed_profit = closed_profit + profit
    
    fee         = fee + leave_price * vm[product_match] * 
                  l_close[j,no_contract] * fee.rate[product_match] + 
                  fee_rate_fix[product_match]
    
    cash        = cash + leave_price * vm[product_match] * 
                  l_close[j,no_contract] - leave_price * 
                  vm[product_match] * l_close[j,no_contract] * 
                  fee.rate[product_match] - fee_rate_fix[product_match]
    
    position[product_match] = position[product_match] - 1 #adjust position
    holding[product_match] = holding[product_match] - l_close[j,no_contract]
...
if(nrow(l_contracts) != 0) {
  ...
  l_contracts <- as.data.table(left_join(l_contracts, information, 
                                         by = "product_name"))
...
if(nrow(s_contracts) != 0) {
  s_contracts <- as.data.table(left_join(s_contracts, information, 
                                         by = "product_name"))
...
if(nrow(s_exit) == 0){
  ...
  for (j in 1:nrow(s_exit)){
    ...
    leave_date  = cdt[ptr,date]
    leave_price = max(cdt[[ptr, 3 + (product_match - 1) * 15]], 
                      s_exit[j,highs20]) + slippage[product_match] 
    ori_direction = s_exit[j,direction]
    commision   = leave_price * vm[product_match] + 
                  enter_price * vm[product_match]
    profit      = (enter_price - leave_price) * vm[product_match] * 
                  s_exit[j,no_contract] 
    closed_profit = closed_profit + profit
    fee     = fee + leave_price * vm[product_match] * s_exit[j,no_contract] * 
                  fee.rate[product_match] + fee_rate_fix[product_match]
    
    cash        = cash - leave_price * vm[product_match] * 
                  s_exit[j,no_contract] - leave_price * 
                  vm[product_match] * s_exit[j,no_contract] * 
                  fee.rate[product_match] - fee_rate_fix[product_match]
    
    position[product_match] = position[product_match] +1 #adjust position
    holding[product_match] = holding[product_match] + s_exit[j,no_contract] 
...
if(nrow(l_exit) == 0){
  ...
  for (j in 1:nrow(l_exit)){
    ...
    leave_date  = cdt[ptr, date]
    leave_price = min(cdt[[ptr, 3 + (product_match - 1) * 15]], 
                      l_exit[j, lows20]) - slippage[product_match]
    ...
    closed_profit = closed_profit + profit
    fee         = fee + leave_price * vm[product_match] * 
                  l_exit[j,no_contract] * fee.rate[product_match] + 
                  fee_rate_fix[product_match]
    
    cash        = cash + leave_price * vm[product_match] * l_exit[j,no_contract] - 
                  leave_price * vm[product_match] * l_exit[j,no_contract] * 
                  fee.rate[product_match] - fee_rate_fix[product_match]
    position[product_match] = position[product_match] - 1 #adjust position
    holding[product_match] = holding[product_match] - l_exit[j,no_contract] 
...
if(nrow(sta_contract_dt) != 0){
  ...
  asset_out <- data.table(
    ...
    pos_profit = sum(a_cont$direction * (a_cont$close - 
                                a_cont$enter_price) * a_cont$no_contract * a_cont$vm),
  ...
  asset_out <- data.table(
    date = cdt[ptr,date], 
    fee  = fee,
    holding_value = sum(na.omit(holding*a_close)), 
...
calc_annual_return <- function(x){ # 根据日度收益率计算年化收益率
  (1 + sum(x)) ^(250 / length(x)) - 1   
  #由于本金永远锁定在Account而非再投资，这里不能用prod而是用sum来简单加总
}
